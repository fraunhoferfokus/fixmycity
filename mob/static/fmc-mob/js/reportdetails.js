...
(function() {
    ...
    function fetchComments(cid, div, offset) {
        ...
        $.ajax({
            url: App.RESOURCE_REPORTS+cid+'/comments/',
    ...
    function doRating(cid, score) {
        $.post(App.RESOURCE_REPORTS+cid+'/ratings/',
            { value : score })
        .success(function() {
            // lock own rating bar
    ...
    // PAGEBEFORESHOW
    $('#reportdetails').live('pagebeforeshow', function() {
        console.log("=== pagebeforeshow for #reportdetails");

        // get report ID from URL
        var report_id = App.parseUri('cid') || 1;

        // get data, fill page
        $.getJSON(App.RESOURCE_REPORTS+report_id+'/', function(data) {
            ...
            if(settings.MAP_PROVIDER=="bing"){
            	mapUrl = BingMap.getStaticMap(data.address, 14, 400, 300);
            }else{
            	mapUrl = GoogleMap.getStaticMap(data.address, 17, 400, 300);
            }
            ...
            $.getJSON(App.RESOURCE_REPORTS+report_id+'/ratings/', {
                ...
                updateOwnRatingDisplay(start, readOnly, report_id);
            });
